...
!   2011-11-07 todling - explicitly read header contents (read type does not work all times)
!
! input argument list:
...
! Declare passed arguments
  ...
  type(diag_header_chan_list),pointer    :: header_chan(:)=>NULL()
...
! Read header (fixed_part).
  read(ftin,IOSTAT=iflag) header_fix%isis,header_fix%id,&
             header_fix%obstype,header_fix%jiter,header_fix%nchan,header_fix%npred,&
             header_fix%idate,header_fix%ireal,header_fix%ipchan,&
             header_fix%iextra,header_fix%jextra,&
             header_fix%idiag,header_fix%angord,header_fix%iversion,header_fix%inewpc
  if (iflag/=0) then
     rewind(ftin)
...
! Read header (channel part)
  do ich=1, header_fix%nchan
     read(ftin,IOSTAT=iflag) header_chan(ich)%freq,     &
...
!   2011-11-07 todling - explicitly read data_extra (read type does not work all times)
!
! input argument list:
!   ftin - unit number connected to diagnostic file
!   header_fix - header information structure
...
! Declare passed arguments
  ...
  type(diag_data_chan_list)  ,pointer    :: data_chan(:)=>NULL()
  type(diag_data_extra_list) ,pointer    :: data_extra(:,:)=>NULL()
  integer(i_kind),intent(out)            :: iflag
    
  integer(i_kind) :: ich,iang,ndiag,ii,jj
...
! Transfer data record to output structure
  ...
  if (header_fix%iextra > 0) then
      do jj = 1,header_fix%jextra
         do ii = 1,header_fix%iextra
            data_extra(ii,jj)%extra=extra_tmp(ii,jj)
         enddo
      enddo
